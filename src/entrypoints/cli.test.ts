...
describe('Node 24 premature exit regression (issue #1678)', () => {
  it('built CLI stays alive during initialization in interactive mode without premature exit', async () => {
    ...
    const scriptPath = `${cacheDir}/test-cli-startup.mjs`
    await Bun.write(scriptPath, `
      ...
      // We do NOT add any setTimeout or manual event loop handles here.
      // We rely entirely on the built CLI's top-level await and its internal handles.
      // If the top-level await is missing, Node 24 may exit prematurely before the TUI attaches.
      import('../dist/cli.mjs');
    `)
    ...
    // Give it 500ms to either exit prematurely (the bug) or stay alive (the fix)
    await new Promise(r => setTimeout(r, 500))
    
    // Assert it hasn't exited prematurely
    expect(proc.exitCode).toBe(null) // null means it is still running
